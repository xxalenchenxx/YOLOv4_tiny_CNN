from checkpoints.CNN import CNN
import torch
...
model = CNN(2)
model.load_state_dict(torch.load('./checkpoints/CNN_54lite_2.pt', map_location = 'cpu'))
model.eval()
...
def CNN(img, bboxes):
    ret = []
    out_boxes, _, _, num_boxes = bboxes
    image_h, image_w, _ = img.shape
    for i in range(num_boxes[0]):
        #print(len(outboxes))
        coor = out_boxes[0][i]
        coor[0] = int(coor[0] * image_h) #y_lu
        coor[2] = int(coor[2] * image_h) #y_rd
        coor[1] = int(coor[1] * image_w) #x_lu
        coor[3] = int(coor[3] * image_w) #x_rd
#         print(f"coor[0]: {coor[0]} , coor[2]: {coor[2]} , coor[1]: {coor[1]} , coor[3]: {coor[3]}")
        data = transform(img[ int(coor[0]):int(coor[2]), int(coor[1]):int(coor[3]) ])
        data = torch.unsqueeze(data, dim = 0)
        pred = model(data)
        _, y = torch.max(pred, 1)
        ret.append(label[int(y)])
    return ret
